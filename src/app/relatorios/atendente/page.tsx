...
export default function AttendantDailyReportPage() {
  ...
  const loadAttendants = async (start: string, end: string) => {
    try {
      const uniqueList = await attendanceService.getUniqueAttendants(start, end);
      ...
      setSelectedUserId((prev) => {
        if (!prev || !attendantOptions.some((a) => a.id_user === prev)) {
          return attendantOptions.length > 0 ? attendantOptions[0].id_user : "";
        }
        return prev;
      });
    } catch (e: unknown) {
  ...
  // Carrega lista de atendentes quando datas mudarem
  useEffect(() => {
